...
import { NonGstBillEditForm } from "@/components/sales/NonGstBillEditForm";

export default function EditNonGstBillPage({ params }: { params: Promise<{ billNumber: string }> }) {
  const { billNumber } = use(params);
  return (
    <RequireRole roles={["admin"]} message="Only admins can edit bills.">
      <EditNonGstBill billNumber={billNumber} />
    </RequireRole>
  );
}

function EditNonGstBill({ billNumber }: { billNumber: string }) {
  const { data: bill, isLoading, isError, refetch } = useGetNonGstBillQuery(billNumber);
  ...
  return (
    <div className="mx-auto max-w-2xl">
      ...
      <p className="mt-1 text-sm text-muted">{billNumber}</p>
      ...
      {justSaved && (
        ...
        >
          ...
          <span className="flex gap-3">
            <Link href={`/invoice/${billNumber}`} target="_blank" className="font-medium text-primary underline">
              View bill
            </Link>
            <Link href="/non-gst-bills" className="font-medium text-primary underline">
              Back to bills
            </Link>
          </span>
        </div>
      ...
      {bill && bill.status === "paid" && (
        ...
        <NonGstBillEditForm key={bill.updatedAt} bill={bill} onSaved={() => setJustSaved(true)} />
      )}
    </div>
  );
}
